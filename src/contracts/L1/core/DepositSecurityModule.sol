...
contract DepositSecurityModule {
    ...
    constructor(
        ...
        if (_dappLink == address(0)) revert ZeroAddress("_dappLink");
        ...
        ATTEST_MESSAGE_PREFIX = keccak256(
            abi.encodePacked(
                keccak256("dappLink.DepositSecurityModule.ATTEST_MESSAGE"),
                block.chainid,
                address(this)
            )
        );
        ...
        PAUSE_MESSAGE_PREFIX = keccak256(
            abi.encodePacked(
                keccak256("dappLink.DepositSecurityModule.PAUSE_MESSAGE"),
                block.chainid,
                address(this)
            )
        );
    ...
    /**
     * Returns whether DAPPLINK.deposit() can be called, given that the caller will provide
    ...
    function canDeposit(uint256 stakingModuleId) external view returns (bool) {
        ...
        bool isDappLinkCanDeposit = DAPPLINK.canDeposit();
        return (
            isModuleActive
            && quorum > 0
            && block.number - lastDepositBlock >= minDepositBlockDistance
            && isDappLinkCanDeposit
        );
    }
    ...
    function depositBufferedEther(
        ...
        DAPPLINK.deposit(maxDepositsPerBlock, stakingModuleId, depositCalldata);
    }
