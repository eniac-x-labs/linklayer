...
import "@/contracts/libraries/MinFirstAllocationStrategy.sol";
import "@/contracts/libraries/SafeMath.sol";
import "@/contracts/libraries/Packed64x4.sol";
import "@/contracts/libraries/SigningKeys.sol";
...
contract NodeOperatorsRegistry is BaseApp {
    ...
    //
    bytes32 internal constant SIGNING_KEYS_MAPPING_NAME = keccak256("dapplink.NodeOperatorsRegistry.signingKeysMappingName");


    /// @dev Total number of operators
    bytes32 internal constant TOTAL_OPERATORS_COUNT_POSITION = keccak256("dapplink.NodeOperatorsRegistry.totalOperatorsCount");

    /// @dev Cached number of active operators
    bytes32 internal constant ACTIVE_OPERATORS_COUNT_POSITION = keccak256("dapplink.NodeOperatorsRegistry.activeOperatorsCount");

    /// @dev link to the index of operations with keys
    bytes32 internal constant KEYS_OP_INDEX_POSITION = keccak256("dapplink.NodeOperatorsRegistry.keysOpIndex");


    bytes32 internal constant STUCK_PENALTY_DELAY_POSITION = keccak256("dapplink.NodeOperatorsRegistry.stuckPenaltyDelay");
    ...
    function addNodeOperator(string memory _name, address _rewardAddress) external returns (uint256 id) {
        ...
        _checkRole(MANAGE_NODE_OPERATOR_ROLE);
    ...
    function setNodeOperatorStakingLimit(uint256 _nodeOperatorId, uint64 _vettedSigningKeysCount) external {
        _onlyExistedNodeOperator(_nodeOperatorId);
        _checkRole(SET_NODE_OPERATOR_LIMIT_ROLE);
    ...
    function updateStuckValidatorsCount(bytes memory _nodeOperatorIds, bytes memory _stuckValidatorsCounts) external {
        ...
        assembly {
            _nodeOperatorIdsOffset := add(calldataload(4), 36) 
            _stuckValidatorsCountsOffset := add(calldataload(36), 36) 
        }
        ...
        external
    {
        ...
        assembly {
            _nodeOperatorIdsOffset := add(calldataload(4), 36) 
            _exitedValidatorsCountsOffset := add(calldataload(36), 36) 
        }
        for (uint256 i; i < nodeOperatorsCount;) {
            /// @solidity memory-safe-assembly
            assembly {
    ...
    function addSigningKeys(uint256 _nodeOperatorId, uint256 _keysCount, bytes memory _publicKeys, bytes memory _signatures) external {
        _addSigningKeys(_nodeOperatorId, _keysCount, _publicKeys, _signatures);
    }


    function _addSigningKeys(uint256 _nodeOperatorId, uint256 _keysCount, bytes memory _publicKeys, bytes memory _signatures) internal {
        _onlyExistedNodeOperator(_nodeOperatorId);
    ...
    function clearNodeOperatorPenalty(uint256 _nodeOperatorId) external {
        Packed64x4.Packed memory stuckPenaltyStats = _loadOperatorStuckPenaltyStats(_nodeOperatorId);
        require(
    ...
    function getNodeOperatorIds(uint256 _offset, uint256 _limit)
        ...
        uint256 nodeOperatorsCount = getNodeOperatorsCount();
        if (_offset >= nodeOperatorsCount || _limit == 0) return new uint256[](0);
    ...
    // function _distributeRewards() internal returns (uint256 distributed) {
        // IStETH stETH = IStETH(locator.lido());
        ...
        // }
        // if (toBurn > 0) {
        //     IBurner(locator.burner()).requestBurnShares(address(this), toBurn);
        // }
    // }
    ...
    function _onlyNodeOperatorManager(address _sender, uint256 _nodeOperatorId) internal view {
        ...
        if(isRewardAddress && isActive){
            _checkRole(MANAGE_SIGNING_KEYS, _sender);
        }
    ...
    function _onlyValidRewardAddress(address _rewardAddress) internal view {
        ...
        require(_rewardAddress != locator.dapplink(), "LIDO_REWARD_ADDRESS");
    }
}