...
import "@/contracts/L1/interface/IStakingModule.sol";
...
import "@/contracts/L1/core/BeaconChainDepositor.sol";
import "@/contracts/L1/core/BaseApp.sol";

contract StakingRouter is BaseApp, BeaconChainDepositor {
    ...
    bytes32 internal constant WITHDRAWAL_CREDENTIALS_POSITION = keccak256("dapplink.StakingRouter.withdrawalCredentials");

    /// @dev total count of staking modules
    bytes32 internal constant STAKING_MODULES_COUNT_POSITION = keccak256("dapplink.StakingRouter.stakingModulesCount");
    /// @dev id of the last added staking module. This counter grow on staking modules adding
    bytes32 internal constant LAST_STAKING_MODULE_ID_POSITION = keccak256("dapplink.StakingRouter.lastStakingModuleId");
    /// @dev mapping is used instead of array to allow to extend the StakingModule
    bytes32 internal constant STAKING_MODULES_MAPPING_POSITION = keccak256("dapplink.StakingRouter.stakingModules");
    /// @dev Position of the staking modules in the `_stakingModules` map, plus 1 because
    ///      index 0 means a value is not in the set.
    bytes32 internal constant STAKING_MODULE_INDICES_MAPPING_POSITION = keccak256("dapplink.StakingRouter.stakingModuleIndicesOneBased");
    ...
    struct NodeOperatorSummary {
        ...
        ///     compensated to the Dapplink by the node operator
        uint256 refundedValidatorsCount;
    ...
    /// @notice Helper for Dapplink contract (DEPRECATED)
    ...
    function getTotalFeeE4Precision() external view returns (uint16 totalFee) {
        /// @dev The logic is placed here but in Dapplink contract to save Dapplink bytecode
    ...
    /// @dev Helper only for Dapplink contract. Use getStakingFeeAggregateDistribution() instead
    ...
    function deposit(
        ...
        if (msg.sender != locator.dapplink()) revert AppAuthDapplinkFailed();
